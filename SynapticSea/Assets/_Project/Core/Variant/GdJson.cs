...
namespace SynapticSea.Core.Variant
{
    ...
    public static class GdJson
    {
        ...
        public static object Parse(string text) => Parse(text, exactNumbers: false);

        /// <summary>
        /// Parses JSON text. <paramref name="exactNumbers"/> = true is NOT Godot behaviour: integer literals (no '.',
        /// no exponent) become exact <c>long</c> values and other numbers use correctly rounded parsing instead of
        ...
        sealed class Parser
        {
            ...
            object ParseValue(int depth)
            {
                ...
                if (c == '-' || (c >= '0' && c <= '9'))
                {
                    ...
                    if (ExactNumbers)
                    {
                        if (lexeme.IndexOfAny(new[] { '.', 'e', 'E' }) < 0 &&

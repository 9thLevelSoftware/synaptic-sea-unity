...
24. **KEEP ithappy structural is an additive kit, not a replacement for `ship_structural_v0`.** Godot `assets/imported/structural/ithappy/` copies into `Assets/Content/Structural/ithappy/` (glTFast; `Frame.cs` is unchanged). Kit id `ithappy_scifi_v0` keeps the v0 `module_id` strings so procgen can swap GLBs without renaming IDs. Point a layout at it with `"kit_id": "ithappy_scifi_v0"` (`KitCatalog.ITHAPPY_KIT_ID`); `ShipSceneBuilder` loads `Resources/Catalogs/KitCatalog_ithappy_scifi_v0`. Build prefabs with `StructuralPrefabBuilder -kit ithappy_scifi_v0` (wrappers in `fixtures/godot_wrappers/ithappy/`, contracts in `StreamingAssets/data/placement/contracts/structural/ithappy_scifi_v0/`, including Art Director `wall_x_junction`). Default generation and goldens stay on `ship_structural_v0`. The full ithappy prop pack is ~2.3 GB (LFS), so only `doors_hatches` + `security` plus `Content/Props/ithappy/inventory.json` are imported; copy more from the Godot repo with `tools/import_ithappy_keep.py`.

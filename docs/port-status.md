...
| 7 Content pipeline | Done (first pass) | 15 v0 structural prefabs, 26 prop prefabs, catalogs, frame convention verified on 41 authored sockets. Additive KEEP ithappy spine lives at `Assets/Content/Structural/ithappy` (same 15 `module_id`s plus `wall_x_junction`); see decision 61. |
| 8 Runtime scene layer | Done (first pass) | `ShipSceneBuilder` builds wrappers, markers, portals, zones, props, dressing and objective volumes, matching Godot loader fixtures. `Assets/Scenes/Playable.unity` (`PlayableSceneBuilder`) boots a `RunSession` through `RunSessionHost` with real scene ports (`Runtime/Session`), interaction/zone/threat/hallucination views, travel and F5/F9; the HUD and menus are wired by `Game/SessionUiBridge`. `PlayableScenePlayModeTests` runs the real loop with the golden ship's threats alive. It covers:<br>• boot, walk, registry interact, route-gate colliders and F5/F9 (including wounds and the web chart);<br>• a real stalker detecting and hitting the player, shown on the HUD, and attack keys killing a threat, reload and hotbar;<br>• making the life boat flyable through repair, fire suppression and breach seal points, then travelling to breach_field (hazard kit), dead_fleet (industrial) and hive (biomatter) derelicts and back;<br>• the gamepad inventory→pause→resume journey, settings persisting from Title through play and pause back to Title, and Title → New Run → quit.<br>`RunLifecyclePlayModeTests` covers Milestone A New Run (golden hub, fail-closed non-slice), Continue, death/extract → `RunResultsPanel` → Title, and failures → Title. Capture: `PlayableScreenshotRunner` |
...
30. **Generated New Run** (superseded for Title New Run by decision 60). Title → New Run still opens a setup for biome, difficulty and seed. `StartSceneBuilder.BuildHomeStart` remains the viability path (EditMode `StartSceneViabilityTests`) and can write `layout.json`, `gameplay_slice.json` and `blueprint.json` to `user://runs/<run_id>/`. The live Title New Run / direct-open boot does not call it: the hub is golden `coherent_ship_001`.
    ...
    - **Tolerated warning.** `RoomAssigner`'s "guaranteed role 'dock' has no eligible zone" warning is logged at info level on this path only (`RoomAssigner.ToleratedMissingRoles`). Godot parity tests and derelict generation keep the warning.
...
59. **Threats walk on a NavMesh, with `NavMeshAgent`s** (`com.unity.ai.navigation` 2.0.14; 2026-09-17), replacing Godot's hand-rolled path following. AI Navigation was deferred in the port plan (step 5) and Godot's own `NavigationRegion3D` bake was dropped as debug-only; this takes it up. Core keeps owning every threat's position, so saves, the tick order and the headless suites are unchanged.
    - **The seam.** `IThreatNavigation` (`RunSessionDeps`) is a scene port like `ILineOfSightProbe`. Each tick `ThreatRuntime.AdvanceThreatMotion` hands the agent a destination and stores the position it reached; `Hold` stops an idle, stunned or in-range threat and `Release` drops a dead one's agent. With no navigation attached, or none with a NavMesh, the ADR-0049 `ShipNavGraph` A* stepping runs exactly as before — that is what every headless session and EditMode test uses, and what covers an agent that cannot be placed. The graph is still built either way: it resolves each threat's room and the FLEE goal (kept for `REPATH_INTERVAL`, so its Dijkstra sweep does not run every frame). `ThreatNavigationSeamTests` covers both sides.
    - **The surface.** Ships are generated at load, so `ShipNavMesh` builds a `NavMeshSurface` per ship root when the active ship changes (about 30 ms for a golden deck) from that ship's Structure-layer colliders. A ship that docks or travels carries its data with it (the root's transform is watched and the data re-placed). No links are generated between decks: neither engine has a walkable deck change (decision 57).
    - **The agent.** `ProjectSettingsBootstrap` registers a "Threat" agent type (radius 0.35, height 1.8, climb 0.3, id 1734437751) in `ProjectSettings/NavMeshAreas.asset`. Unity's built-in Humanoid (radius 0.5) does not fit through a ship's 1.2 m doorways and seals every room off. 0.35 is the player's own CharacterController radius, and climb 0.3 its step offset.
    - **Doors and gates carve, they are not baked.** A closed portal blocker, a closed route gate and a blocked-route marker each carry a carving `NavMeshObstacle` (`NavMeshBlocker`) that follows its collider's `enabled`, because a baked wall would keep blocking a door the player just unlocked. The structural wrapper of a LOCKED/HATCH portal is marked `ignoreFromBuild` so the carve is the only thing in the doorway.
    - **Fire is costly, not solid.** The nav graph charges `FIRE_COST_MULT` (×6) to cross a burning room; the scene marks those cells with the `ThreatFire` NavMesh area at cost 6 (`NavMeshModifierVolume` per cell, rebuilt when the burning set changes — a fire starting or dying, not every frame). Two things do NOT carry over: the ×0.35 discount for breached rooms (NavMesh area costs cannot go below 1), and a sealed bulkhead that has no collider of its own in the scene — both still apply to the graph, which is what the FLEE goal and the room lookup read.
    - **Paused runs stop the agents by hand** (`RunSessionHost.Update`): they walk in the engine's own update, which a paused simulation does not stop.
    - `ThreatNavMeshPlayModeTests` covers the built surface, a room never being cut in two, a threat walked into range of the player by its agent, a closed door not being walkable, and a burning cell taking the fire area and giving it back.
60. **Milestone A New Run / first-away launch contract** (2026-09-17, Systems Designer lock + REQ-SLICE-001). Supersedes decision 30 for Title New Run and a direct-open Playable scene.
    - New Run does **not** use `StartSceneBuilder`. Hub is golden `coherent_ship_001`. The title setup still offers biome / difficulty / seed; it always opens on the slice defaults (seed 17, `breach_field`, `standard`), ignoring a persisted settings difficulty, so a leftover `hardened` / `deep_dive` preference cannot fail-close New Run. Title `Launch` does not copy the settings difficulty onto a New Run request. An explicit non-slice choice still fails closed (`non_slice_launch: …`) and returns to Title — the wrong layout is never loaded silently.
    - Continue / LoadSlot boot the saved home's `layout_path` (and a missing `gameplay_slice_path` is derived from that directory). They do not replace the save with the Milestone A hub. A save with no usable layout is rejected instead of silently loading `coherent_ship_001`.
...
61. **KEEP ithappy structural is an additive kit, not a replacement for `ship_structural_v0`.** Godot `assets/imported/structural/ithappy/` copies into `Assets/Content/Structural/ithappy/` (glTFast; `Frame.cs` is unchanged). Kit id `ithappy_scifi_v0` keeps the v0 `module_id` strings so procgen can swap GLBs without renaming IDs. Point a layout at it with `"kit_id": "ithappy_scifi_v0"` (`KitCatalog.ITHAPPY_KIT_ID`). The kit document has a complete wrapper map under `res://scenes/wrappers/structural/ithappy/`, so decision 24's `KitPathForLayout` keeps that kit. `CatalogIdForKitDocument` keys the wrapper folder `ithappy`; `ForKitDocument` then loads `Resources/Catalogs/KitCatalog_ithappy_scifi_v0` by kit id when no `KitCatalog_ithappy` exists. Until `StructuralPrefabBuilder -kit ithappy_scifi_v0` fills that catalog, `KitPrefabCatalog.TryGetPrefab` falls back to v0 prefabs for the 15 shared IDs (`wall_x_junction` still needs the bake). KEEP wall wrappers use v0 BoxShape3D wings so the first bake does not drop 4×4×4 cubes into walkable cells. The edge compiler still emits T-junctions for 4-way walls (v0 goldens); `wall_x_junction` is available for authored placements. Default generation stays on `ship_structural_v0`. The full ithappy prop pack is ~2.3 GB (LFS), so only `doors_hatches` + `security` plus `Content/Props/ithappy/inventory.json` are imported; copy more from the Godot repo with `tools/import_ithappy_keep.py`. The `wall_x_junction` GLB is the Art Director pack as-is (pivot may need a prefab recenter; do not regenerate the mesh).
62. **Death and extract open `RunResultsPanel`** (2026-09-17, Systems Designer lock). Godot `title_main.gd` `_on_gameplay_slice_completed` → `_show_run_results`. Unity: `SessionUiBridge` subscribes to `RunSession.PlayableSliceCompleted` (death via `EndRun("death")`, extract via `EndRun("extraction")` / slice `reason=complete`) and opens the TERMINAL results surface. Confirm writes `RunReturnInfo` and loads Title. Pause quit still returns to Title without a results panel. `RunReturnInfo` is the last-run line after confirm — not a substitute for the panel. Verify: `docs/playtest/milestone-a-run-results.md`.
63. **Kit / contract rows are socket authority; new modules need `{module_id}.asset.json`.** `ModularSocketCatalog` and `StructuralEdgeCompiler` load `data/placement/contracts/structural/{kit}/*_contract.json` only — they do not invent `SOCK_*` names. `StructuralPrefabBuilder` reads the kit row (footprint, `socket_names`, pivot, nav) plus that contract, and now also the Forge companion beside the GLB. `KitAuthorityAudit` (EditMode / `dotnet test --filter KitAuthorityAudit`) flags a module **ungated** when kit or contract sockets/footprint are missing, when a kit `SOCK_` name is absent from the contract, or when a new module (no v0 twin) has mesh but no companion. `wall_x_junction.asset.json` is the first companion (`SOCK_wall_face_{north,east,south,west}_01`). Inherited Godot contract extras (`floor_top`, `wall_base`, ceiling sockets) are notes, not ungated. `wall_face` stays out of `ENCLOSURE_KINDS` for Godot bind parity (see `docs/playtest/ithappy-scifi-bake.md`). Deviltop bake: `F:\Unity\6000.6.0f1\Editor\Unity.exe -batchmode -nographics -projectPath SynapticSea -executeMethod SynapticSea.EditorTools.Content.StructuralPrefabBuilder.BuildAll -quit -kit ithappy_scifi_v0 -logFile builds/logs/ithappy-scifi-bake.log`. Bake complete = report `errors: []`, catalog 16 prefabs under `Content/Prefabs/Structural/ithappy_scifi_v0/`, no v0 fallback for shared ids, `wall_x_junction` resolves.
64. **Ithappy Art Director iso stills** (`IthappyKitContactSheet.Run`, 2026-09-17). Instantiates all 16 baked `ithappy_scifi_v0` prefabs on an 8 m grid. Style-gate lock: orthographic size 18–22 (sheet 22 / per-module 18), pitch 35.264° down, yaw bias 45° (Godot equal-axis offset through Frame, not play 16/18/16). Writes `artifacts/screenshots/ithappy_scifi_v0/contact_sheet.png` plus per-module PNGs. GPU required: batchmode **without** `-nographics`. Command: `F:\Unity\6000.6.0f1\Editor\Unity.exe -batchmode -projectPath SynapticSea -executeMethod SynapticSea.EditorTools.Content.IthappyKitContactSheet.Run -quit -logFile builds/logs/ithappy-contact.log`.
...
- Milestone A New Run / first away: implemented (decision 60). Non-slice seed/biome/difficulty fail closed. Verify path in `docs/playtest/milestone-a-new-run.md`.
- Milestone A death / extract → `RunResultsPanel`: implemented (decision 62). Verify path in `docs/playtest/milestone-a-run-results.md`.

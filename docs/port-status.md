...
- Playable: near ceilings stay opaque (Godot's documented rule), so the iso camera sees the player's own ceiling; the loader's `BlockedRoute_*` markers stay collidable after the powered gates open (both as in Godot; confirm against a Godot run).

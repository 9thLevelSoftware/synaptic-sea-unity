...
60. **Milestone A New Run / first-away launch contract** (2026-09-17, Systems Designer lock + REQ-SLICE-001). Supersedes decision 30 for Title New Run and a direct-open Playable scene.
    - New Run does **not** use `StartSceneBuilder`. Hub is golden `coherent_ship_001`. The title setup still offers biome / difficulty / seed; it opens on the slice defaults (seed 17, `breach_field`, `standard`). A non-slice choice fails closed (`non_slice_launch: …`) and returns to Title — the wrong layout is never loaded silently.
